...
if (file.exists("outputs/logs/batch_units.rds")) {
    ...
    # Summary by state
    state_summary <- aggregate(SOx ~ State, data = units, FUN = sum)
    state_summary <- state_summary[order(-state_summary$SOx), ]
    kable(head(state_summary, 10), caption = "Top 10 States by SOx Emissions")
} else {
...
| Met data | NCEP Reanalysis | ~2.5° resolution |
...
|------------|------------|---------------|
| 0.1° | Fine spatial detail | Larger file sizes, slower |
| 0.25° | Good balance | Standard choice |
| 0.5° | Fast processing | May miss local hotspots |
