...
**For HYSPLIT dispersion runs**, install the SplitR package (provides HYSPLIT binaries):
...
`create_dirs()` sets up the project folder structure and defines global path variables.
Run this at the start of each session:
 
```{r}
disperseR::create_dirs(location = "~/disperseR_project")
```
