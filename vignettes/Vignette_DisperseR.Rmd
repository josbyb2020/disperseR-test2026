---
title: "DisperseR: Getting Started"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{DisperseR: Getting Started}
...
This vignette walks through a complete disperseR analysis for US power plants.
We cover directory setup, data downloads, dispersion runs, and exposure calculations.

## Setup

disperseR uses `data.table` for performance. Load the required packages:
...
**For HYSPLIT dispersion runs**, HYSPLIT binaries are required. If the
`splitr` package is installed, disperseR will use its bundled binaries;
otherwise provide `binary_path` and `parhplot_path` to `hysplit_dispersion()`.

## Directory Structure

`create_dirs()` sets up the project folder structure and caches the paths for
use by disperseR functions. Run this at the start of each session:
 
```{r, purl = FALSE}
dirs <- disperseR::create_dirs(location = "~/disperseR_project")

# Extract directory paths for use throughout this vignette
...
Windows note: use a short path (example `C:/disperseR_project`) to avoid
path-length limits.

This creates:

- `main/input/zcta_500k/` - ZCTA shapefiles
- `main/input/hpbl/` - planetary boundary layer data
- `main/input/meteo/` - reanalysis meteorology files
- `main/output/hysplit/` - dispersion outputs
- `main/output/ziplinks/` - ZIP code linkages
- `main/output/rdata/` - HyADS matrices
- `main/output/exp/` - exposure tables
- `main/output/graph/` - plots

Existing data is preserved; only missing directories are created.

## Loading Data

`get_data()` downloads required datasets. Meteorology files are ~120 MB each.

For our November 2005 -- February 2006 analysis window, we download meteorology
one extra month on each side (October 2005 -- March 2006) to cover the 120-hour
(5-day) forward trajectories that may extend beyond the analysis months:
...
# Load ancillary data objects into the workspace
pblheight    <- disperseR::get_data("pblheight")
...
This package contains annual emissions and stack height data from [EPA's Air Markets Program Data](https://campd.epa.gov/) and the [Energy Information Agency](https://www.eia.gov/electricity/data/eia860/) for years 2003-2012. Again, if you would like to know how these data were prepared please see the `Vignette_Units_Preparation` vignette.
...
Note: `input_refs$ID` is used in output filenames and must be filesystem-safe
...
`link_all_units()` can be set to parallelize the runs by splitting different months on different cores, but you can make it serial just by setting the `mc.cores = 1`. The result of `link_all_units()` is a data.table of ZIP codes, counties, or grids and relative contributions `N`, and an identical `.csv` file is saved to the folder with a path specified by the `ziplink_dir` variable. `N` is not weighted by the `unit`'s emissions. 

Below we present how to use `link_all_units()` for chosen units. First, we create an argument (called `yearmons` here) denoting the year and month combinations for which we would like to calculate unweighted exposure. Here we choose November 2005 through February 2006 to match the analysis period defined above, and we create the variable using the `get_yearmon()` function.
...
`pbl.height = pblheight` by default but you can change it. The same applies to `crosswalk. = crosswalk` (`crosswalk.` is only required for linking to ZIP codes). `counties.` is required for linking to counties; you can build a county `sf` object from the `maps` package (which is already a dependency) as shown below. If you'd only like to save time and only link to ZIP codes or counties in a subset of the country, provide `crosswalk.` or `counties.` that correspond to the domain of interest.

```{r, purl = FALSE}
# Build a US counties sf object from the maps package
us_counties_sf <- sf::st_as_sf(maps::map("county", plot = FALSE, fill = TRUE))
```
...
`zoom = TRUE` by default and is automatically adjusted depending on the plot values. You can set `zoom = FALSE` to display the map of the whole country. Note that the linked grids extend beyond the US borders because any grid across the world may be impacted by the given facilities.

```{r, fig.height = 7, fig.width = 7, purl = FALSE}
...
We can plot the results as follows using the `plot_impact_weighted()` function. Again, a customized title can be set using the `plot.name` argument, and additional arguments can be passed to ggplot2's `theme` function to customize. 

Same as before, `zoom = TRUE` by default and is automatically adjusted depending on the plot values. You can set `zoom = FALSE` to display the map of the whole country. 
...
zip_exp_mon_plot <- disperseR::plot_impact_weighted(
  ...
  map.month = "200511", 
  metric = 'hyads',
  legend.name = 'Monthly HyADS exposure', 
  zoom = T, # TRUE by default
  graph.dir = graph_dir)
...
```{r, fig.height = 4, fig.width = 7, purl = FALSE}
...
Please note graphical output is automatically saved to the `graph_dir` by the plotting functions. 
