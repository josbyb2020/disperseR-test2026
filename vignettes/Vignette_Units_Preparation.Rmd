...
Power [Plant Emissions Data](https://doi.org/10.7910/DVN/M3D2NR) and emission data from the [US Environmental Protection Agency (NEI 2014)](https://www.epa.gov/air-emissions-inventories/2014-national-emissions-inventory-nei-data). 
...
# Download '2014fd_inputs_point.zip' from the EPA NEI 2014 portal
# and save it to proc_dir before continuing.
...
The other data set is hosted on the
[Harvard Dataverse](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/M3D2NR).
We specify the direct-access URL and download it the same way.

```{r, eval = FALSE, purl = FALSE}
url <- "https://dataverse.harvard.edu/api/access/datafile/:persistentId?persistentId=doi:10.7910/DVN/M3D2NR/QKLA2U"
...
Many of the units in the provided datasets do not have stack height data. In these cases, it is suggested in [Henneman et al. (2019)](https://doi.org/10.1016/j.atmosenv.2019.01.043) to fill with the average stack height of all units. We do that and create a variable that flags if the height was imputed. 
...
get_units_data <- function(year, d_ampd) {
  ...
  ampd <- ampd[, .(
    ...
    Height = conv_unit(stkhgt, "ft", "m"),
    Diam = conv_unit(stkdiam, "ft", "m"),
    Velocity = conv_unit(stkvel, "ft_per_sec", "m_per_sec"),
    Temp = conv_unit(stktemp, "F", "K")
    )]
