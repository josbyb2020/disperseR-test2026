...
In this vignette, we show how we prepared the crosswalk dataset that is attached to the `disperseR` package. In case you want to use a different crosswalk you can follow some of these preprocessing steps. 
...
dirs <- disperseR::create_dirs()
main_dir <- dirs$main_dir
```

If the file does not exist, download it from the
[UDS Mapper archive](https://github.com/chris-prener/uds-mapper).
Then, open it.

```{r,  eval = FALSE, purl = FALSE}
filename <- file.path(main_dir, "crosswalk.xlsx")
if(!file.exists(filename)){
  download.file("https://raw.githubusercontent.com/chris-prener/uds-mapper/main/data/zip_to_zcta_2017.xlsx", filename)
}
...
We use 2017 census data from [data.census.gov](https://data.census.gov) (ACS 2017 5-year table B01003).
